...
const opts = event => ({ name: event?.contract ? A.label(event.contract, 'Saratoga Springs') : '', tiers: { urgency: D.urgencyTier, expiration: D.expirationTier }, formatDate: D.formatDeadlineDate });
...
test('screenshot scenario: 400 current past the deadline, 180 next year, one contract missing a balance', () => {
  ...
  assert.equal(card.title, 'Use 400 points by Nov 30, 2026');
  assert.equal(card.detail, 'Saratoga Springs');
...
test('banking event says what to bank and by when', () => {
  ...
  const card = attentionCard(event, opts(event));
  assert.equal(card.title, 'Bank 120 points by Nov 30, 2026');
...
test('holding points keep their booking restriction on a second line', () => {
  ...
  const card = attentionCard(event, opts(event));
  assert.match(card.title, /^Use 10 points by /);
  assert.match(card.note, /within 60 days of check-in/);
...
test('a missing current balance names the contract; never an all-clear', () => {
  ...
  const card = attentionCard(event, opts(event));
  assert.equal(card.title, 'Add a balance for Family');
  assert.equal(card.action.href, 'account.html?contract=a&year=2025');
