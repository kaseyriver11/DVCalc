...
test('home attention card and ledger use the expiration scale for use-by states', () => {
  ...
  const useBy = { kind: 'use-by', contract: { id: 'a' }, year: 2025, points: 40, holding: 0, expiresMs: 0, daysUntil: 69 };
  assert.equal(attentionCard(useBy, { name: 'A', tiers, formatDate: () => '' }).tone, 'warning'); // never calm
  const bankable = { kind: 'bankable', contract: { id: 'a' }, year: 2025, points: 40, deadline: { ms: 0, daysUntil: 69 } };
  assert.equal(attentionCard(bankable, { name: 'A', tiers, formatDate: () => '' }).tone, 'calm');
