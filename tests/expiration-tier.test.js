...
vm.runInContext(fs.readFileSync(require.resolve('../js/dvc-dates.js'), 'utf8'), ctx);
...
test('home attention card and ledger use the expiration scale for use-by states', () => {
  const { attentionCard } = require('../js/dvc-home-summary.js');
  ...
  assert.match(fs.readFileSync(require.resolve('../js/home.js'), 'utf8'), /tiers: \{ urgency: urgencyTier, expiration: expirationTier \}/);
