...
| 5 | Banking/borrowing use year planner | Each contract shows its real next deadline in My Contracts (client-side, live now). Contracts also track a manually-maintained points balance (points remaining/banked/borrowed, `db/migrations/006_add_contract_points_tracking.sql`) — edited in My Contracts, and the calendar's "Booking As" card shows what a selected stay would leave in that balance. Reminder emails via a Supabase Edge Function + Resend are coded (`supabase/functions/`) but need the site owner to deploy them — see `docs/phase5_deployment.md` | Built; balance tracking live, reminder emails pending deployment |
