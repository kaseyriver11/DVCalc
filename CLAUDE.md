...
- "Booking As" (contract picker) and "Load Trip" (saved-itinerary picker) live in the top control bar (`#booking-as-control`/`#itinerary-load-control` in index.html, populated by `renderBookingAsControl()`/`renderItineraryLoadControl()`) instead of the summary panel — they're persistent settings like Resort/Room Type/Points Year, not stay-specific output. Only the per-stay eligibility/points feedback (`buildContractEligibilityHTML()`) still lives in the "Your Stay" card
- **Two-phase layout ("review mode")**: once a complete single-resort stay is picked, `isReviewMode()` (`!isSplitMode() && getStayDates().length > 0 && !forceExpandCalendar`) collapses the full calendar into a compact "trip rail" card (`renderTripRail()`, into `#trip-rail-view`) with a mini per-night points strip and an "Edit dates" button (`expandCalendarForEditing()`, sets `forceExpandCalendar = true`), while the summary panel expands (`.summary-side.expanded`) into a wider grid: Cost Comparison leads, then `Your Stay`+`Resort Alerts` and `Booking Outlook`+`Crowd Forecast` pair up via `.insights-grid-2`, then Stay Insights full-width. `forceExpandCalendar` resets to `false` (re-collapsing) whenever a fresh stay gets completed or cleared. Split-stay mode never enters review mode — the full calendar stays expanded throughout, unchanged. `renderLayoutMode()` (called from the tail of `renderCalendar()`/`renderSummary()`) does the show/hide toggling between `#full-calendar-view` and `#trip-rail-view`
