...
| 5 | Banking/borrowing use year planner | Each contract shows its real next deadline in My Contracts. Contracts also track a manually-maintained points balance (points remaining/banked/borrowed, `db/migrations/006_add_contract_points_tracking.sql`) — edited in My Contracts, and the calendar's "Booking As" card shows what a selected stay would leave in that balance. Reminder emails (Supabase Edge Functions + Resend, `supabase/functions/`) were deployed and verified live 2026-09-05 against `dvcalc.app`, with a daily `pg_cron` schedule — see `docs/phase5_deployment.md` | Done |
